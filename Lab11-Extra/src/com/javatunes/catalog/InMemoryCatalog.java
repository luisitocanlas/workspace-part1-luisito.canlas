...
public class InMemoryCatalog implements Catalog {
    ...
    @Override
    public MusicItem findById(Long id) {    // let's say a 12 came in here
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        ...
        for (MusicItem item : catalogData) {
            if (item.getArtist().equalsIgnoreCase(keyword) || item.getTitle().equalsIgnoreCase(keyword)) {
                result.add(item);
            }
        }
        return result;
    ...
    public Collection<MusicItem> findByCategory(MusicCategory category) {
        // declare a return value, a holding are for matches
        Collection<MusicItem> result = new ArrayList<>();
    ...
    public MusicItem cheapItem(MusicCategory category) {
        ...
        for (MusicItem item : catalogData) {
            if (item.getMusicCategory().equals(category)) {
                items.add(item);                                // add items from the category in the created array
            }
        }

        MusicItem cheap = items.get(0);                         // initialize the first item from the list

        for (MusicItem item : items) {
            if (item.getPrice() <= cheap.getPrice()) {          // go through the list and get the cheapest one
                cheap = item;
            }
        }
        return cheap;
     ...
     */
    public double avgCostGenre(MusicCategory category) {
     ...
     */
    public boolean isGenreAvailable(double minPrice) {
        boolean result = false;
